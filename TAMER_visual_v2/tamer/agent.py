...
MOUNTAINCAR_ACTION_MAP = {0: 'left', 1: 'Car_NONE', 2: 'right'}
...
class Tamer:
    ...
    async def train(self, model_file_to_save=None):
        ...
        for i in range(self.num_episodes):
            rew=self._train_episode(i, disp)
            reward_training.append(rew)
            ep_train.append(i)
        ...
        return reward_training,ep_train
    ...
    def play(self, n_episodes=1, render=False):
        ...
        self.env.close()
        return ep_rewards,ep_num

    def evaluate(self, n_episodes=100):
        print('Evaluating agent')
        rewards,ep_num= self.play(n_episodes=n_episodes)
        avg_reward = np.mean(rewards)
        reward_episode_ev=[]
        reward_episode_ev.append(rewards)
        print(
            f'Average total episode reward over {n_episodes} '
            f'episodes: {avg_reward:.2f}'
        )
        return reward_episode_ev,ep_num
